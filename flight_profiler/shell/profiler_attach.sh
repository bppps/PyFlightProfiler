...
if [ "$IS_DARWIN" = "false" ]; then
	...
	fi
	{
		...
		echo "set \$f = (int (*)(char *, int, unsigned long))dlsym(\$m, \"profiler_attach\")"
...
else
	{
		...
		echo "expr int (* \$func)(char*, int, unsigned long) = (int (*)(char*, int, unsigned long))dlsym(\$handle, \"profiler_attach\")"
